...
const BallpitCanvas = dynamic(() => import('@/ultilites/Ballpit'), { ssr: false });

export default function Footer({ config }: { config?: Record<string, string> }) {
  return (
    <footer
      className="relative overflow-hidden border-t border-[#d5e2fa] pt-16 pb-8 text-gray-700"
      style={{
        background: 'linear-gradient(180deg, rgba(245,248,253,0.96) 0%, rgba(223,232,246,0.96) 100%)',
      }}
    >
      ...
      <div className="relative z-20 max-w-[1440px] mx-auto px-2 md:px-4 grid grid-cols-1 md:grid-cols-3 gap-10">
        ...
        <div>
          ...
          <p className="text-[14px] text-gray-700 leading-relaxed pr-6">
        ...
        <div>
          ...
          <ul className="flex flex-col gap-3 text-[14px]">
            {[
              ...
              <li key={l.href}>
                <Link href={l.href} className="text-gray-700 hover:text-[#2563EB] hover:translate-x-1 inline-block transition-all duration-300 font-medium">{l.label}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="flex flex-col gap-4 text-[14px] text-gray-600">
            <li className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-full bg-white/90 shadow-sm flex items-center justify-center shrink-0 border border-blue-50">
            ...
            <li className="flex items-center gap-3">
              ...
              <span className="font-medium text-gray-800">{config?.contact_phone || '024.6682.8899'}</span>
            </li>
            <li className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-white/90 shadow-sm flex items-center justify-center shrink-0 border border-blue-50">
      ...
      <div className="relative z-20 max-w-[1440px] mx-auto px-2 md:px-4 mt-12 pt-6 border-t border-[#d5e2fa] flex flex-col md:flex-row items-center justify-between gap-4">
